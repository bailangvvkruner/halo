...
- 写操作路由已开始接入权限中间件
- 后台已补 P0 前端闭环入口：评论回复管理、注册验证操作页
- 评论回复页已补审核按钮（通过 / 拒绝），注册页已补 token 验证操作
- 更多写接口已统一接入权限中间件，前端已补 401/403 请求失败反馈
...
- 后台 UI 重构第一步已补：文章管理页改成更接近原版后台的工具栏 + 搜索框 + 表格式列表
- 后台 UI 重构第二步已补：页面管理、分类管理、标签管理改成统一工具栏 + 搜索框 + 表格式列表
- 后台 UI 重构第三步已补：评论管理、回复审核、用户管理、插件管理、主题管理、附件管理都改成统一表格式后台页
- 后台 UI 重构第四步已补：控制台新增顶部标题栏与快捷操作区，整体更接近原版后台框架
- 后台 UI 重构第五步已补：文章编辑页、页面编辑页骨架已加入控制台导航，形成基础编辑流入口
- 后台 UI 重构第六步已补：文章编辑页支持保存草稿/发布，页面编辑页支持保存与发布状态切换
